  ...
  return function connect() /* [options], select, DecoratedComponent */{
    ...
    var ConnectorDecorator = React.createClass({
      ...
      selectState: function (props, context) {
        ...
        invariant(_.isPlainObject(slice), 'The return value of `select` prop must be an object. Instead received %s.', slice);

        return slice;
      },

      render: function () {
        return React.createElement(DecoratedComponent, _.assign({ ref: 'decoratedComponent' }, this.nextState, this.props));
      }
    });
